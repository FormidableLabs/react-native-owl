//
//  UIScrollView+OwlActions.m
//  OwlClient
//
//  Created by Rob Walker on 23/11/2021.
//

#import "UIScrollView+OwlActions.h"
...
@interface UIScrollView (OwlScrolling)
...
@implementation UIScrollView (OwlActions)
...
static BOOL _DTXApplyScroll(UIScrollView* scrollView, CGPoint startPoint, CGPoint offset, CGPoint* remainingOffset)
{
    ...
    if(points.count > 1)
    {
        ...
        [OwlSyntheticEvents touchAlongPath:points relativeToWindow:scrollView.window holdDurationOnFirstTouch:0.0 holdDurationOnLastTouch:0.0 onTouchCallback:^ BOOL (UITouchPhase phase) {
            ...
            if(consecutiveTouchPointsWithSameContentOffset >
               OwlPolicy.consecutiveTouchPointsWithSameContentOffsetThreshold)
            {
                if(didSomeScroll == NO)
                {
                    didFailTouches = YES;
