...
@implementation OwlRunLoopSpinner
{
  BOOL _spinning;
}
...
- (BOOL)spinWithStopConditionBlock:(BOOL (NS_NOESCAPE ^)(void))stopConditionBlock {
  ...
  [self owl_drainRunLoopInActiveModeForDrains:_minRunLoopDrains];
  BOOL stopConditionMet = [self owl_checkConditionInActiveMode:stopConditionBlock];
  CFTimeInterval remainingTime = [self owl_secondsUntilTime:timeoutTime];
  
  while (!stopConditionMet && remainingTime > 0) {
    @autoreleasepool {
      stopConditionMet = [self owl_drainRunLoopInActiveModeAndCheckCondition:stopConditionBlock
                                      forTime:remainingTime];
      remainingTime = [self owl_secondsUntilTime:timeoutTime];
    }
  }
 ...
 */
- (void)owl_drainRunLoopInActiveModeForDrains:(NSUInteger)exitDrainCount {
  if (exitDrainCount == 0) {
    return;
  }
  ...
  while (drainCount < exitDrainCount) {
    @autoreleasepool {
      NSString *activeMode = [self owl_activeRunLoopMode];
      CFRunLoopObserverRef drainCountingObserver = [self owl_setupObserverInMode:activeMode
      ...
      [self owl_teardownObserver:drainCountingObserver inMode:activeMode];
    }
  }
}
 ...
 */
- (BOOL)owl_drainRunLoopInActiveModeAndCheckCondition:(BOOL (^)(void))stopConditionBlock
                         forTime:(CFTimeInterval)time {
  NSString *activeMode = [self owl_activeRunLoopMode];
  CFRunLoopTimerRef wakeUpTimer = [self owl_setupWakeUpTimerInMode:activeMode];
  ...
  [self owl_setupObserverInMode:activeMode
  ...
  if (result == kCFRunLoopRunFinished) {
    ...
    conditionMet = [self owl_checkConditionInActiveMode:stopConditionBlock];
  }
  
  [self owl_teardownObserver:conditionCheckingObserver inMode:activeMode];
  [self owl_teardownTimer:wakeUpTimer inMode:activeMode];
 ...
 */
- (BOOL)owl_checkConditionInActiveMode:(BOOL (^)(void))stopConditionBlock  {
  __block BOOL conditionMet = NO;
  __weak __typeof__(self) weakSelf = self;
 ...
 */
- (CFRunLoopObserverRef)owl_setupObserverInMode:(NSString *)mode
 ...
 */
- (CFRunLoopTimerRef)owl_setupWakeUpTimerInMode:(NSString *)mode {
 ...
 */
- (void)owl_teardownObserver:(CFRunLoopObserverRef)observer inMode:(NSString *)mode {
  if (observer) {
 ...
 */
- (void)owl_teardownTimer:(CFRunLoopTimerRef)timer inMode:(NSString *)mode {
  if (timer) {
 ...
 */
- (CFTimeInterval)owl_secondsUntilTime:(CFTimeInterval)time {
