...
//DTX_DIRECT_MEMBERS
@implementation UIView (OwlUtils)
...
- (BOOL)_dtx_isTestedRegionObscuredWithVisiblePixels:(NSUInteger)visible
    ...
    if (isRegionObscured) {
        *explanation = [NSString stringWithFormat:@"View does not pass visibility percent "
                        "threshold (%@)", [OwlPolicy percentDescriptionForValue:percent]];
    }
...
//                                 "threshold (%@)",
//                                 [OwlPolicy percentDescriptionForValue:percent]];
//        
...
//        return NO;
        ...
        NSMutableDictionary* details = [NSMutableDictionary dictionary];
                [details setValue: [NSString stringWithFormat:@"View is clipped by one or more of its "
                                    "superviews' bounds and does not pass visibility percent "
                                    "threshold (%@)",
                                    [OwlPolicy percentDescriptionForValue:percent]] forKey:NSLocalizedDescriptionKey];
                
        NSError* err = [NSError errorWithDomain:@"DetoxErrorDomain" code:0 userInfo:details];
        _DTXPopulateError(err);
    ...
    if ([self _dtx_isTestedRegionObscured:testedRegionInWindowCoords
        ...
        NSMutableDictionary* details = [NSMutableDictionary dictionary];
                [details setValue: [NSString stringWithFormat:@"View is obscured by its window bounds and does not pass visibility percent threshold (%@)",
                                    [OwlPolicy percentDescriptionForValue:percent]] forKey:NSLocalizedDescriptionKey];
    ...
    NSUInteger visible = [image
        dtx_numberOfVisiblePixelsWithAlphaThreshold:OwlPolicy.visibilityPixelAlphaThreshold
        totalPixels:&total];
...
- (BOOL)dtx_isVisibleAtRect:(CGRect)rect percent:(nullable NSNumber *)percent
    ...
    NSUInteger percentValue = percent ? percent.unsignedIntegerValue :
        OwlPolicy.defaultPercentThresholdForVisibility;
