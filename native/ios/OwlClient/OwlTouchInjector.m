...
- (instancetype)initWithWindow:(UIWindow *)window onTouchInectCallback:(nullable BOOL(^)(UITouchPhase))callback
{
  ...
  if (self)
  {
    ...
    _state = kOwlTouchInjectorPendingStart;
...
- (void)startInjecting
{
  ...
  if (_state == kOwlTouchInjectorStarted)
  {
    return;
  }
  
  _state = kOwlTouchInjectorStarted;
  if(_displayLink == nil)
  {
...
- (void)waitUntilAllTouchesAreDeliveredUsingInjector
{
  ...
  // Start if necessary.
  if (_state == kOwlTouchInjectorPendingStart || _state == kOwlTouchInjectorStopped)
  {
    [self startInjecting];
  }
  
  // Now wait for it to finish.
  OwlRunLoopSpinner *runLoopSpinner = [OwlRunLoopSpinner new];
  ...
  {
    return (_state == kOwlTouchInjectorStopped);
  }];
}
...
- (void)displayLinkDidTick
{
  ...
  OwlTouchInfo *touchInfo = [self owl_dequeueTouchInfoForDeliveryWithCurrentTime:CACurrentMediaTime()];
  if (!touchInfo)
  {
    if (_enqueuedTouchInfoList.count == 0)
    {
      // Queue is empty - we are done delivering touches.
      [self owl_stopTouchInjection];
    }
    return;
  }
  ...
  if ([_ongoingUITouches count] == 0)
  {
    ...
    [self owl_extractAndChangeTouchToStartPhase:touchInfo];
  }
  else if (touchInfo.phase == OwlTouchInfoPhaseTouchEnded)
  {
    reportedPhase = UITouchPhaseEnded;
    [self owl_changeTouchToEndPhase:touchInfo];
  }
  else
  {
    ...
    {
      [self owl_changeTouchToMovePhase:touchInfo];
    }
  }
  [self owl_injectTouches:touchInfo];
  ...
  if(_callback && _abortedByCallback == NO)
  {
    ...
    if(_abortedByCallback)
    {
      ...
      [_enqueuedTouchInfoList filterUsingPredicate:[NSPredicate predicateWithFormat:@"phase == %@", @(OwlTouchInfoPhaseTouchEnded)]];
    }
  }
}
 ...
 */
- (UITouch *)owl_UITouchForFinger:(NSUInteger)index
{
 ...
 */
- (void)owl_extractAndChangeTouchToStartPhase:(OwlTouchInfo *)touchInfo
{
 ...
 *  @param touchInfo The info that is used to create the UITouch.
 */
- (void)owl_changeTouchToEndPhase:(OwlTouchInfo *)touchInfo
{
  for (NSUInteger i = 0; i < [touchInfo.points count]; i++)
  {
    UITouch *touch = [self owl_UITouchForFinger:i];
 ...
 *  @param touchInfo The info that is used to create the UITouch.
 */
- (void)owl_changeTouchToMovePhase:(OwlTouchInfo *)touchInfo
{
  for (NSUInteger i = 0; i < [touchInfo.points count]; i++)
  {
 ...
 *  @param touchInfo The info that is used to create the UITouch.
 */
- (void)owl_injectTouches:(OwlTouchInfo *)touchInfo
{
  ...
  {
    UITouch *currentTouch = [self owl_UITouchForFinger:i];
    if (i == 0)
    {
  ...
  @autoreleasepool
  {
    ...
    @finally
    {
      ...
      if (touchInfo.phase == OwlTouchInfoPhaseTouchEnded)
      {
 ...
 */
- (void)owl_stopTouchInjection
{
  _state = kOwlTouchInjectorStopped;
 ...
 *          @c nil is returned.
 */
- (OwlTouchInfo *)owl_dequeueTouchInfoForDeliveryWithCurrentTime:(CFTimeInterval)currentTime
{
  ...
  OwlTouchInfo* rv = [_enqueuedTouchInfoList firstObject];
  if(rv != nil)
  {
